...
/*
  ...
  * Ëµ    Ã÷£ºÒÚÁÖ ·§¿ØÆ÷¿ØÖÆº¯Êý£¬ÔÚ´Ëº¯ÊýÖÐÊµÏÖÒÚÁÖ·§¿Ø²»Í¬¿ØÖÆ¡£
  *					
  * ÊäÈë²ÎÊý£º 
...
uint8 ValveContron_Elsonic(MeterFileType *p_mf,uint8 functype,uint8 *p_DataIn,uint8 *p_databuf,uint8 *p_datalenback)
{
	...
	switch(functype){
		case ReadVALVE_All:{						
			Create_ElsonicVave_ReadInfo_Frame(p_mf,lu8dataframe, &lu8datalen);//×é½¨»ñÈ¡·§ÃÅÊý¾ÝÃüÁîÖ¡
...
//ÏÂÃæÆÁ±Î²¿·ÖÊÇÎÂ¿Ø¼ÆÁ¿Ò»Ìå»¯ÖÐÓÃµÄ´úÂë¶Î¡£			
// 			if(Err == NO_ERR){	  
// 				l8tmp = (int8)lu8dataframe[4];  //·¿¼äÎÂ¶È+²¹³¥ÎÂ¶È¡£
...
// 				lu8tmp = lu8dataframe[7];
// 				if(lu8tmp & 0x02)  //ÎÞÏßÒì³£¡£
// 					lu8databuf[4] |= 0x01;

// 				lu8tmp = lu8dataframe[3];
// 				if((lu8tmp & 0x01) == 0)  //Ãæ°å¿ª¹Ø
// 					lu8databuf[4] |= 0x02;

// 				if(lu8tmp & 0x04)  //Ãæ°åËø¶¨ÊÇ·ñ¡£
// 					lu8databuf[4] |= 0x04;
...
// 			*p_datalenback = lu8datalenback;
		...
		case SETHEAT_DISPLAY:{
			Create_ElsonicVave_SetInfo_Frame(p_mf,lu8dataframe, &lu8datalen);	//ÉèÖÃÇëÓÃÈÈÁ¿ÏÔÊ¾¡£		  
			Err = Elsonic_ReadData_Communicate(lu8dataframe, &lu8datalen);
			if(Err != NO_ERR)  {
		...
		case SETVALVE_CONTROLTYPE:{
			if(*(p_DataIn+0) == 0x09){ //ÒÚÁÖ·§Ö»ÓÐÖØÐÂÊ¹ÄÜ·§¿ØÆ÷ÓÉÎÂ¿ØÃæ°å¿ØÖÆÊ±²ÅÖ´ÐÐ¡£
				Create_ElsonicVave_SetValve_Frame(p_mf,p_DataIn,lu8dataframe, &lu8datalen);
				Err = Elsonic_ReadData_Communicate(lu8dataframe, &lu8datalen);
		...
		case READVALVE_OPENTIME:{
			Create_ElsonicVave_ReadOpenTime_Frame(p_mf,p_DataIn,lu8dataframe, &lu8datalen);
			Err = Elsonic_ReadData_Communicate(lu8dataframe, &lu8datalen);
			memcpy(p_databuf,lu8dataframe,lu8datalen);
			*p_datalenback = lu8datalen;
		...
		case READVALVE_WATERTEMP:{
			Create_ElsonicVave_ReadWaterTemp_Frame(p_mf,p_DataIn,lu8dataframe, &lu8datalen);
			Err = Elsonic_ReadData_Communicate(lu8dataframe, &lu8datalen);
			memcpy(p_databuf,lu8dataframe,lu8datalen);
			*p_datalenback = lu8datalen;
   ...
   /*
	 ******************************************************************************
	 * º¯ÊýÃû³Æ£ºCreate_ElsonicVave_SetInfo_Frame(DELU_Protocol *pData, uint8 *pSendFrame, uint8 *plenFrame)
	 * Ëµ	 Ã÷£º´Ëº¯ÊýÓÃÓÚ¶ÁÈ¡ÒÚÁÖ·§¿ØÆ÷×´Ì¬ÐÅÏ¢¡£
	 * ²Î	 Êý£º 
	 ******************************************************************************
	 */
   ...
   /*
	 ******************************************************************************
	 * º¯ÊýÃû³Æ£ºCreate_ElsonicVave_SetInfo_Frame(DELU_Protocol *pData, uint8 *pSendFrame, uint8 *plenFrame)
	 * Ëµ	 Ã÷£º´Ëº¯ÊýÓÃÓÚ½«ÉèÖÃÒÚÁÖ·§¿ØÆ÷Ê¹ÄÜÈÈÁ¿ÏÔÊ¾¹¦ÄÜ¡£
	 * ²Î	 Êý£º 
	 ******************************************************************************
	 */
   void Create_ElsonicVave_SetInfo_Frame(MeterFileType *p_mf, uint8 *pSendFrame, uint8 *plenFrame)
   {
		   ...
		   setInform[1] = lu8valveaddr[0];
		   setInform[2] = lu8valveaddr[1];
		   setInform[3] = 0x08; // ÆôÓÃÈÈÁ¿ÏÔÊ¾¹¦ÄÜ¡£
   ...
   /*
	 ******************************************************************************
	 * º¯ÊýÃû³Æ£ºCreate_ElsonicVave_SetHeat_Frame(DELU_Protocol *pData, uint8 *pSendFrame, uint8 *plenFrame)
	 * Ëµ	 Ã÷£º´Ëº¯ÊýÓÃÓÚ½«ÈÈÁ¿ÖµÐ´ÈëÒÚÁÖ·§¿ØÆ÷¡£
	 * ²Î	 Êý£º 
	 ******************************************************************************
	 */
...
//		   uint32 lu32currentheat =0;
		   ...
		   setInform[5] = BCD_TO_HEX(*(p_DataIn + 1));  //Éè¶¨ÎÂ¶È,´Ë´¦ÐèÒªµ÷ÕûÕýÈ·Æ«Á¿¡£
		   if(setInform[5] < 10)	//ÎÂ¶ÈÉè¶¨·¶Î§10-30¡æ¡£
			   setInform[5] = 10;
		   if(setInform[5] > 30)
			   setInform[5] = 30;
		   
		   setInform[5] = 2 * setInform[5]; 	 //  Éè¶¨ÊÒÄÚÎÂ¶È£¬Éè¶¨ÖµÊÇÎÂ¶ÈÖµµÄ2±¶¡£
   ...
   /*
	 ******************************************************************************
	 * º¯ÊýÃû³Æ£ºCreate_ElsonicVave_SetRoomTempRange_Frame(DELU_Protocol *pData, uint8 *pSendFrame, uint8 *plenFrame)
...
//		   uint32 lu32currentheat =0;
		   ...
		   setInform[4] = BCD_TO_HEX(*(p_DataIn+1));  //Éè¶¨ÎÂ¶ÈÉÏÏÞÖµ¡£
		   if(setInform[4] < 20)	//ÉÏÏÞ·¶Î§20-30¡æ¡£
			   setInform[4] = 20;
		   if(setInform[4] > 30)	//ÉÏÏÞ·¶Î§20-30¡æ¡£
			   setInform[4] = 30;
   
		   setInform[4] = 2 * setInform[4]; 	 //  Éè¶¨ÖµÊÇÎÂ¶ÈÖµµÄ2±¶¡£
   
		   setInform[6] = BCD_TO_HEX(*(p_DataIn+4));  //Éè¶¨ÎÂ¶ÈÏÂÏÞÖµ¡£
		   if(setInform[6] < 10)	//ÉÏÏÞ·¶Î§10-19¡æ¡£
			   setInform[6] = 10;
		   if(setInform[6] > 19)   
			   setInform[6] = 19;
   
		   setInform[6] = 2 * setInform[6]; 	 //  Éè¶¨ÖµÊÇÎÂ¶ÈÖµµÄ2±¶¡£
   ...
   /*
	 ******************************************************************************
	 * º¯ÊýÃû³Æ£ºCreate_ElsonicVave_SetValve_Frame(DELU_Protocol *pData, uint8 valvestate,uint8 *pSendFrame, uint8 *plenFrame)
...
//		   uint32 lu32currentheat =0;
		   ...
		   if(*p_DataIn == 0x99)
			   ...
			   setInform[3] = 0x02;
		   else if(*p_DataIn == 0x09)  //·§Õý³£¹¤×÷£¬»¹ÊÇÓÉÎÂ¿ØÃæ°å¿ØÖÆ¡£
		   	 	setInform[3] = 0;
		   else
			   setInform[3] = 0x02;//ÆäËûÒ²È«²¿´ò¿ª·§ÃÅ£¬°²È«¡£
      ...
      /*
	 ******************************************************************************
	 * º¯ÊýÃû³Æ£ºCreate_ElsonicVave_ReadOpenTime_Frame(DELU_Protocol *pData, uint8 valvestate,uint8 *pSendFrame, uint8 *plenFrame)
	 * Ëµ	 Ã÷£º´Ëº¯ÊýÓÃÓÚ¶ÁÈ¡ÒÚÁÖ·§¿ØÆ÷µÄ¿ª·§Ê±¼ä¡£
	 * ²Î	 Êý£º 
	 ******************************************************************************
	 */
   void Create_ElsonicVave_ReadOpenTime_Frame(MeterFileType *p_mf, uint8 *p_DataIn,uint8 *pSendFrame, uint8 *plenFrame)
   {
...
/*
******************************************************************************
* º¯ÊýÃû³Æ£ºCreate_ElsonicVave_ReadWaterTemp_Frame(DELU_Protocol *pData, uint8 valvestate,uint8 *pSendFrame, uint8 *plenFrame)
* Ëµ	 Ã÷£º´Ëº¯ÊýÓÃÓÚ¶ÁÈ¡ÒÚÁÖ·§¿ØÆ÷µÄ¿ª·§Ê±¼ä¡£
* ²Î	 Êý£º 
******************************************************************************
*/
 void Create_ElsonicVave_ReadWaterTemp_Frame(MeterFileType *p_mf, uint8 *p_DataIn,uint8 *pSendFrame, uint8 *plenFrame)
 {
	uint8 setInform[8]={0xaf,0x19,0x0c,0x00,0x00,0x00,0x00,0x7b};
	uint8 cs 	   = 0x00;
	uint8 *pTemp;
	uint8 i =0;
	uint8 lu8valveaddr[7] = {0x00};
